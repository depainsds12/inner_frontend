...
const PositiveToggle = ({ className = "", points, setPoints }) => {
  ...
  return (
    <div className="relative h-[32px] w-[150px]">
      <div
        className={`h-[32px] w-[150px] ${value < 50 ? "bg-yellow-light" : "bg-white"} relative p-[2px] ${className} transition-all duration-150`}
        ref={sliderRefOuter}
      >
        <div
          className={`pseudo-clip-path z-0 grid h-full w-full select-none place-items-center ${value < 50 ? "bg-[#8958B4] text-yellow-light" : value > 50 ? "bg-black text-white" : "bg-[#564A8D]"} mx-auto text-[14px] font-extrabold transition-all duration-150`}
          ref={sliderRef}
        >
          {value < 50 ? "activate" : value > 50 ? "deactivate" : ""}
        </div>
      </div>
      ...
      >
        <div className="relative">
          <h1
            ...
            className={`absolute left-0 grid h-[44px] w-[44px] translate-x-[-50%] place-items-center bg-no-repeat ${value < 50 ? "bg-[url('/assets/journal-flow/flower-purple-icon.svg')]" : value > 50 ? "bg-[url('/assets/journal-flow/flower-grey-icon.svg')]" : "bg-[url('/assets/journal-flow/positive-toggle-flower.svg')]"} ${
